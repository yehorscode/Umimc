...
# Defining debug_mode() default = False
debug_mode = True
...
# Loop
if debug_mode == True:
    debug("Not showing welcome screen due to Debug Mode activation")
else:
    for i in range(1):
        print(f"""{Fore.GREEN}

    This app uses Modrinth as Backend
...
if action == "1":
    ...
    # Debbuging Responses
    debug(f"[REQUEST] Trying to request Labrinth at: {url_get_findrequest + request_modname + url_get_findrequest_string_type_mod, Fore.RESET}")
...
#    print(fmod_dict)
    ...
    debug(debug_text+"[REQUEST] Requested labrinth at "+url_fmod_labrinthapi+Fore.RESET)
    ...
    debug(debug_text+"[REQUEST] Requested labrinth at "+"https://api.modrinth.com/v2/project/"+fmod_chosenid+Fore.RESET)
    ...
    print()
    print()
    loader_continue = input(f"""{Back.BLUE}Do You want to continue? I found loaders: {find_loader}.
Yes or No? (y/n){Back.RESET}""")
    
    if loader_continue.lower == "y" or loader_continue.lower == "yes":
        print(Fore.GREEN+"Okay"+Fore.RESET)
    else:
        print(Back.RED+"Stopping!"+Back.RESET)
        quit()
    ...
    # Correct! ↓
    if len(project_id) == 8:
        debug(debug_text+"Checking project id"+Fore.RESET)
        ...
        if labrinthapi_project_info.status_code == 200:
            debug(debug_text+"Request was succesfull"+Fore.RESET)
        else:
            print(Back.RED+"Something is wrong!"+Back.RESET)
            print(Back.RED+labrinthapi_project_info.status_code+Back.RESET)
